<script>
  ...
  async function handleVNC(name) {
    try {
      ...
      const { port, host } = await res.json();
      window.open(`http://${host}:${port}/vnc.html`, '_blank');
    } catch (error) {
      console.error('Failed to open VNC:', error);
    }
  }

  async function handleDetachISO(name) {
    try {
      await fetch(`/api/vms/${name}/detach-iso`, { method: 'POST' });
      await loadVMs();
    } catch (error) {
          ...
          <td class="py-2 space-x-2">
            {#if vm.state === 'running'}
              ...
              <button
                ...
                on:click={() => handleDetachISO(vm.name)}
              >
                Detach ISO
              </button>
            ...
            <button
              ...
              disabled={stoppingVMs.has(vm.name)}
            >
              Delete
            </button>
          </td>
